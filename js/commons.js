...
var menu = menuWrapper.querySelector('.menu');
var btn = document.querySelector('.header__menu-btn');
var subMenuItems = menuWrapper.querySelectorAll('.sub-menu');
// class names
var openedClass = 'page--modal-opened';
var closedClass = 'page--modal-closed';
var fixedClass = 'header__container--fixed';
var menuItemOpenedClass = 'menu__item--opened';
var menuItemActiveClass = 'menu__item--active';
...
function main() {
    ...
    menu.addEventListener('transitionend', function (e) {
        ...
        var isClosed = page.classList.contains(closedClass);
    ...
    ;[].concat(_toConsumableArray(subMenuItems)).forEach(function (sub) {
        var li = sub.parentElement;
        var link = li.querySelector(' [class*="__link"]');
        // Toogle sub-menu
        link.addEventListener('click', function () {
            sub.style.maxHeight = !sub.style.maxHeight ? sub.scrollHeight + 'px' : null;
            li.classList.toggle(menuItemOpenedClass);
        });
        // Open sub-menu on __item--active
        // TODO: handle sub-menu__item
        if (li.classList.contains(menuItemActiveClass)) link.dispatchEvent(new CustomEvent('click'));
    });
...
function stickyMenu() {
    ...
    var handleIntersect = function handleIntersect(entries) {
        var isOpened = page.classList.contains(openedClass);
...
function toggleModal(e) {
    ...
    var prevScrollPos = page.dataset.scrollPos;
...
function handleEscape(e) {
    if (e.defaultPrevented) return;
    var isModalOpened = document.querySelector('.' + openedClass);
    if (isModalOpened && event.keyCode === 27) toggleModal();
}

function setOffset(el) {
    var scrollBarWidth = document.body.offsetWidth - window.innerWidth;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var _svg$url$split = _blank2.default.url.split('#'),
        _svg$url$split2 = _slicedToArray(_svg$url$split, 1),
        commonUrl = _svg$url$split2[0];

    var elms = document.body.querySelectorAll('use');

    fetch(commonUrl).then(function (res) {
        return res.text();
        ...
        // replace svg url
        (0, _replaceUrl2.default)(commonUrl);
    });
    // add loading class
    Array.from(elms).filter(function (el) {
